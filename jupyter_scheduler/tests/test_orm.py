...
def test_completed_cells_column_migration(jp_scheduler_db_url):
    ...
    completed_cells_column = next(
        col for col in inspector.get_columns("jobs") if col["name"] == "completed_cells"
    )
    ...
    job = Job(runtime_environment_name="test_env", input_filename="test.ipynb", completed_cells=5)
    session.add(job)
    session.commit()
