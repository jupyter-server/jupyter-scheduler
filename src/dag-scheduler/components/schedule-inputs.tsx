...
>(props: ScheduleInputsProps<M, E>): JSX.Element | null {
  ...
  const timezones = useMemo(() => supportedTimeZones.sort(), []);
...
export function cronToSchedule(cron: string): ModelWithScheduleFields {
  const [minute, hour, monthDay, , weekDay] = parseCron(cron);
