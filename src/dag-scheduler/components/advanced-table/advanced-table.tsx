...
>(props: AdvancedTableProps<P, Q, R>): JSX.Element {
  ...
  const [filterFuncs, setFilterFuncs] = useState<any[]>();
  const [filteredRows, setFilteredRows] = useState<R[]>();
  ...
  const fetchInitialRows = async () => {
    ...
    props
      ...
      .then(payload => {
        ...
        setMaxPage(rows.length % pageSize);
      })
      .catch((e: Error) => {
  ...
  useEffect(() => {
    ...
    const filteredRows = rows?.filter(row =>
      filterFuncs?.length ? filterFuncs?.every(func => func(row)) : true
    );
    ...
    setMaxPage(totalRows % pageSize);
  }, [rows, filterFuncs]);

  const handelFilterUpdate = (predicates: any[]) => {
    setFilterFuncs(predicates);
  };
  ...
  const renderedRows: JSX.Element[] = useMemo(
    ...
    [filteredRows, props.rowFilter, props.renderRow, page, pageSize]
  );
  ...
  const labelDisplayedRows = useCallback(
    ({ from, to, count }: LabelDisplayedRowsArgs) => {
      if (count === -1) {
        const loadedRows = filteredRows?.length ?? 0;
        if (onLastPage) {
    ...
    >
      ...
      >
        <TableFilter
          ...
          onFilterChange={handelFilterUpdate}
        />
