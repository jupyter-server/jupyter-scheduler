...
function JobParameters(props: {job: Scheduler.IDescribeJob}): JSX.Element | null {
      ...
      <div className={valueClass}>
        {Object.keys(params).map((paramName, idx) =>
          <p className={'jp-notebook-job-parameter'} key={idx}>
            <span className={'jp-notebook-job-parameter-name'} style={{fontWeight: 'bold'}}>{paramName}: </span>
            <span className={'jp-notebook-job-parameter-value'}>{params[paramName]}</span>
          </p>
        )}
      </div>
    </div>
...
function OutputFormats(props: {job: Scheduler.IDescribeJob}): JSX.Element | null {
  if (props.job.output_formats === undefined || props.job.output_formats === null) {
    return null;
  }
...
export function JobDetails(props: JobDetailsProps) {
  ...
  const end_date: Date | null = props.job.end_time ? new Date(props.job.end_time) : null;
  const end_display_date: string | null = end_date ? end_date.toLocaleString() : null;

  if (!props.isVisible) {
    return <div className='jp-notebook-job-details details-hidden' />;
  }

  return <div className='jp-notebook-job-details details-visible'>
